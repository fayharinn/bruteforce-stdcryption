...
class CryptoFooter:
	...
	def dump(self):
		...
		print ('Crypto Type    :'+"blah")
		print ('Encrypted Key  :', "0x" + str(binascii.hexlify(self.cryptoKey)))
		print ('Salt           :', "0x" + str(binascii.hexlify(self.cryptoSalt)))
...
def main(args):
	...
	if len(args) < 3:
		...
		headerData = open(headerFile, 'rb').read(64)
...
def decryptDecodePbkdf2Key(cf,password):
	...
	pbkdf2 = EVP.pbkdf2(password.encode(), cf.cryptoSalt, iter=HASH_COUNT, keylen=KEY_LEN_BYTES+IV_LEN_BYTES)

	rkey = pbkdf2[:KEY_LEN_BYTES]
	#print(rkey)
	iv = pbkdf2[KEY_LEN_BYTES:]

	# do the decrypt
	cipher = EVP.Cipher(alg='aes_128_cbc', key=rkey, iv=iv, op=0) # 0 is DEC
